...
// Mirrors mooncake/mooncake-store/src/wds/wds_backend.cpp:39-131 field-for-field
// so a single config file works for both consumers of the vendor lib.
bool
...
// Per-xfer key override: caller serializes one key per descriptor as a
// '\n'-delimited blob in opt_args->customParam. Returns true and fills `out`
// only when the count matches `expected_count` exactly. WDS KVCache rejects
// overwrite of an existing key, so initiators that need a fresh key per
// iteration (e.g. nixlbench WRITE warmup+bench) use this channel to bypass the
// key cached at registerMem time.
bool
...
nixlWQSKVEngine::nixlWQSKVEngine(const nixlBackendInitParams *init_params)
    ...
    const std::string conf_path = resolveConfigPath(init_params);
    if (conf_path.empty()) {
    ...
    std::call_once(g_init_once, [&]() {
        KVCacheOptions opts;
        if (!loadKVCacheOptionsFromJson(conf_path, opts)) {
...
nixlWQSKVEngine::prepXfer(const nixl_xfer_op_t &operation,
    ...
    if (!isValidPrepXferParams(operation, local, remote)) {
    ...
    if (opt_args != nullptr) {
        req_h->have_per_iter_keys =
            parseCustomParamKeys(opt_args->customParam, local.descCount(), req_h->per_iter_keys);
    }
